//

package blssig12_test

import (
	"encoding/hex"
	"errors"
	"testing"

	"github.com/stretchr/testify/assert"
	"github.com/stretchr/testify/require"
	blst "github.com/supranational/blst/bindings/go"

	"github.com/valkyrieworks/dffc4ef0-4ec6-4346-9b6c-d0a0cb42d94b/security"
	"github.com/valkyrieworks/dffc4ef0-4ec6-4346-9b6c-d0a0cb42d94b/security/signature381"
)

func TestNewPrivateKeyFromBytes(t *testing.T) {
	privKey, err := signature381.GenPrivKey()
	require.NoError(t, err)
	defer privKey.Zeroize()

	privKeyBytes := privKey.Bytes()
	privKey2, err := signature381.NewPrivateKeyFromBytes(privKeyBytes)
	require.NoError(t, err)
	defer privKey2.Zeroize()

	assert.True(t, privKey.Equals(privKey2))

	_, err = signature381.NewPrivateKeyFromBytes(security.CRandBytes(31))
	assert.Error(t, err)
}

func TestGenPrivateKey(t *testing.T) {
	privKey, err := signature381.GenPrivKey()
	require.NoError(t, err)
	defer privKey.Zeroize()
	assert.NotNil(t, privKey)
}

func TestGenPrivKeyFromSecret(t *testing.T) {
	secret := []byte("REDACTED")
	privKey, err := signature381.GenPrivKeyFromSecret(secret)
	require.NoError(t, err)
	assert.NotNil(t, privKey)
}

func TestGenPrivKeyFromSecret_SignVerify(t *testing.T) {
	secret := []byte("REDACTED")
	priv, err := signature381.GenPrivKeyFromSecret(secret)
	require.NoError(t, err)
	msg := []byte("REDACTED")
	sig, err := priv.Sign(msg)
	require.NoError(t, err)

	pub := priv.PubKey()
	assert.True(t, pub.VerifySignature(msg, sig), "REDACTED")
}

func TestPrivKeyBytes(t *testing.T) {
	privKey, err := signature381.GenPrivKey()
	require.NoError(t, err)
	defer privKey.Zeroize()

	privKeyBytes := privKey.Bytes()
	privKey2, err := signature381.NewPrivateKeyFromBytes(privKeyBytes)
	require.NoError(t, err)
	defer privKey2.Zeroize()

	assert.True(t, privKey.Equals(privKey2))
}

func TestPrivKeyPubKey(t *testing.T) {
	privKey, err := signature381.GenPrivKey()
	require.NoError(t, err)
	pubKey := privKey.PubKey()
	assert.NotNil(t, pubKey)
}

func TestPrivKeyEquals(t *testing.T) {
	privKey, err := signature381.GenPrivKey()
	require.NoError(t, err)
	privKey2, err := signature381.GenPrivKey()
	require.NoError(t, err)

	assert.True(t, privKey.Equals(privKey))
	assert.False(t, privKey.Equals(privKey2))
}

func TestPrivKeyType(t *testing.T) {
	privKey, err := signature381.GenPrivKey()
	require.NoError(t, err)
	defer privKey.Zeroize()

	assert.Equal(t, "REDACTED", privKey.Type())
}

func TestPrivKeySignAndPubKeyVerifySignature(t *testing.T) {
	privKey, err := signature381.GenPrivKey()
	require.NoError(t, err)
	defer privKey.Zeroize()
	pubKey := privKey.PubKey()

	msg := security.CRandBytes(32)
	sig, err := privKey.Sign(msg)
	require.NoError(t, err)

	//
	assert.True(t, pubKey.VerifySignature(msg, sig))

	//
	//
	sig[7] ^= byte(0x01)

	assert.False(t, pubKey.VerifySignature(msg, sig))

	msg = security.CRandBytes(192)
	sig, err = privKey.Sign(msg)
	require.NoError(t, err)

	//
	assert.True(t, pubKey.VerifySignature(msg, sig))
}

func TestPubKey(t *testing.T) {
	privKey, err := signature381.GenPrivKey()
	require.NoError(t, err)
	defer privKey.Zeroize()
	pubKey := privKey.PubKey()
	assert.NotNil(t, pubKey)
}

func TestPubKeyEquals(t *testing.T) {
	privKey, err := signature381.GenPrivKey()
	require.NoError(t, err)
	pubKey := privKey.PubKey()
	pubKey2 := privKey.PubKey()

	assert.True(t, pubKey.Equals(pubKey))
	assert.True(t, pubKey.Equals(pubKey2))
}

func TestPubKeyType(t *testing.T) {
	privKey, err := signature381.GenPrivKey()
	require.NoError(t, err)
	defer privKey.Zeroize()
	pubKey := privKey.PubKey()

	assert.Equal(t, "REDACTED", pubKey.Type())
}

func TestConst(t *testing.T) {
	privKey, err := signature381.GenPrivKey()
	require.NoError(t, err)
	defer privKey.Zeroize()
	assert.Equal(t, signature381.PrivKeySize, len(privKey.Bytes()))

	pubKey := privKey.PubKey()
	assert.Equal(t, signature381.PubKeySize, len(pubKey.Bytes()))

	msg := security.CRandBytes(32)
	sig, err := privKey.Sign(msg)
	require.NoError(t, err)
	assert.Equal(t, signature381.SignatureLength, len(sig))
}

func TestPrivKey_MarshalJSON(t *testing.T) {
	privKey, err := signature381.GenPrivKey()
	require.NoError(t, err)
	defer privKey.Zeroize()

	jsonBytes, err := privKey.MarshalJSON()
	require.NoError(t, err)

	privKey2 := new(signature381.PrivKey)
	err = privKey2.UnmarshalJSON(jsonBytes)
	require.NoError(t, err)
}

func TestPubKey_MarshalJSON(t *testing.T) {
	privKey, err := signature381.GenPrivKey()
	require.NoError(t, err)
	defer privKey.Zeroize()
	pubKey, _ := privKey.PubKey().(signature381.PubKey)

	jsonBytes, err := pubKey.MarshalJSON()
	require.NoError(t, err)

	pubKey2 := new(signature381.PubKey)
	err = pubKey2.UnmarshalJSON(jsonBytes)
	require.NoError(t, err)
}

func TestPubKey_NewPublicKeyFromInvalidBytes(t *testing.T) {
	unmarshal := func(s string) ([]byte, error) {
		type blstPublicKey = blst.P1Affine

		bz, err := hex.DecodeString(s)
		if err != nil {
			return nil, err
		}
		pk := new(blstPublicKey).Uncompress(bz)
		if pk == nil {
			return nil, signature381.ErrDeserialization
		}
		pkc := pk.Serialize()
		if pkc == nil {
			return nil, errors.New("REDACTED")
		}
		return pkc, nil
	}

	testCases := []struct {
		desc        string
		pkStr       string
		expectedErr error
	}{
		{"REDACTED", "REDACTED", signature381.ErrInfinitePubKey},
		{"REDACTED", "REDACTED", signature381.ErrDeserialization},
		{"REDACTED", "REDACTED", signature381.ErrDeserialization},
	}

	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			bz, err := unmarshal(tc.pkStr)
			if err != nil {
				t.Log(tc.desc, "REDACTED", err)
				require.Equal(t, tc.expectedErr, err)
			}

			_, err = signature381.NewPublicKeyFromBytes(bz)
			require.Equal(t, tc.expectedErr, err)
			t.Log(tc.desc, "REDACTED", err)
		})
	}
}
